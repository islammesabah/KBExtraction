...
def _coerce_to_result(obj: Any, sentence: str) -> ExtractionResult:
    ...
    Falls back to empty edges if shape is wrong.
    """
    empty_result: ExtractionResult = {"sentence": sentence, "edges": []}
    ...
    # normalize edges to list[TripletSOP]
    norm_edges: list[TripletSOP] = []
        ...
        for item in edges:
            ...
            if isinstance(item, (list, tuple)) and len(item) == 3:
                ...
                if all(isinstance(x, str) for x in (subject, object, rel)):
                    ...
                    norm_edges.append((subject.strip(), object.strip(), rel.strip()))
            # also accept dicts like {"subject": "...", "object": "...", "relation": "..."}
                ...
                if all(isinstance(x, str) for x in (subject, object, rel)):
                    norm_edges.append((str(subject).strip(), str(object).strip(), str(rel).strip()))

    return { "sentence": str(out_sentence), "edges": norm_edges }
